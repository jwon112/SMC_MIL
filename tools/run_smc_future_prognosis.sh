...
usage() {
  echo "Usage: bash $0 --gpu ID --task-root PATH [--feature-root PATH] [--folds 3] [--max-epochs 30] [--scale SCALE]"
}
...
EXP_CODE="smc_future_significant_${SCALE}_uni2_clamsb_gold${FOLDS}cv_fixed${MAX_EPOCHS}"
...
echo "[DONE] results/${EXP_CODE}_s1"
