#!/usr/bin/env bash
set -euo pipefail
GPU=""; WORKER="all"; FEATURE_ROOT="data/features/uni_v2"; MANIFEST_ROOT=""; RESULTS_ROOT="results/smc_event_stain_pilot"; SEEDS=(1)
while [[ $# -gt 0 ]]; do
  case "$1" in
    --gpu) GPU="$2"; shift 2;; --worker) WORKER="$2"; shift 2;; --feature-root) FEATURE_ROOT="$2"; shift 2;; --manifest-root) MANIFEST_ROOT="$2"; shift 2;; --results-root) RESULTS_ROOT="$2"; shift 2;; --seeds) shift; SEEDS=(); while [[ $# -gt 0 && "$1" != --* ]]; do SEEDS+=("$1"); shift; done;; *) echo "Unknown argument: $1" >&2; exit 2;;
  esac
done
