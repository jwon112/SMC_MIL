...
class CLAM_SB(nn.Module):
    ...
    def forward(
        ...
        A_raw = A                         # 🔒 raw attention 저장 (softmax 전)

        # 2. 🔥 blur-based weight modulation (softmax 이전)
        if weights is not None:
            # weights: [N] → [1, N]
            w = weights.unsqueeze(0).to(A.device)
            A = A * w
...
class CLAM_MB(CLAM_SB):
    ...
    def forward(
        ...
        # 🔥 blur-based attention modulation (softmax 이전)
        if weights is not None:
            w = weights.unsqueeze(0).to(A.device)  # [1, N]
            A = A * w
