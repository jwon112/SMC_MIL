...
from .swinUNet.fp16_util import convert_module_to_f16, convert_module_to_f32
from .swinUNet.nn import (
    SiLU,
    conv_nd,
    linear,
    avg_pool_nd,
...
class SwinTransformerBlock(TimestepBlock):
    ...
    def __init__(self, dim, input_resolution, num_heads, window_size=7, shift_size=0,
                 ...
                 act_layer=nn.GELU, norm_layer=nn.LayerNorm,time_embed_dim=16,out_channels = 64):
        super().__init__()
        # print("SwinTransformerBlock time_embed_dim ",time_embed_dim)
        # print("SwinTransformerBlock input_resolution ",input_resolution)
        # print("SwinTransformerBlock out_channels ",out_channels)
        self.dim = dim
        ...
        # print("swin out self.emb_layers ",self.emb_layers)
    ...
    def _forward(self, x,emb):
        ...
        # print("SwinTransformerBlock x ",x.shape)
        shortcut = x.clone()
...
class BasicLayer(TimestepBlock):
    ...
    def __init__(self, dim, input_resolution, depth, num_heads, window_size,
                 ...
                 drop_path=0., norm_layer=nn.LayerNorm, downsample=None, use_checkpoint=False,time_embed_dim=16,out_channels = 64):
...
class BasicLayer_up(TimestepBlock):
    ...
    def __init__(self, dim, input_resolution, depth, num_heads, window_size,
                 mlp_ratio=4., qkv_bias=True, qk_scale=None, drop=0., attn_drop=0.,
                 drop_path=0., norm_layer=nn.LayerNorm, upsample=None, use_checkpoint=False,out_channels = 64,time_embed_dim=256):
...
class SwinUNetModel(ModelMixin, ConfigMixin):
    @register_to_config
    def __init__(
        self,
        ...
        time_embed_dim = pow(int(sample_size / patch_size), 2)
        ...
        self.patch_embed = PatchEmbed(
            img_size=sample_size, patch_size=patch_size, in_chans=self.in_channels, embed_dim=embed_dim,
            norm_layer=norm_layer if self.patch_norm else None)
        ...
        for i_layer, mult in enumerate(self.depths):
            for _ in range(num_res_blocks):
                ...
                layers = BasicLayer(dim=int(embed_dim * 2 ** i_layer),
                                    ...
                                    out_channels = int(sample_size / (pow(2, i_layer + 2))),
            ...
            for _ in range(num_res_blocks):
                ...
                if i_layer ==0 :
                    ...
                    layer_up = BasicLayer_up(dim=int(embed_dim * 2 ** (self.num_layers-1-i_layer)),
                                             ...
                                             use_checkpoint=use_checkpoint,
                                             out_channels = int(sample_size / (pow(2, 5 - i_layer))),
                                             time_embed_dim = time_embed_dim)
        ...
        if self.final_upsample == "expand_first":
                ...
                self.up = FinalPatchExpand_X4(input_resolution=(sample_size // patch_size, sample_size // patch_size), dim_scale=4, dim=embed_dim)
    ...
    def forward(self, x, timesteps, y=None,return_dict=False):

        if timesteps.shape.__len__()==0:
        ...
        # print("embed timesteps.shape ",emb.shape)
